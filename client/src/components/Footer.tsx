...
export default function Footer() {
  const { storeName, storePhone, codEnabled, razorpayEnabled, freeShippingThreshold, copyFooterTagline, socialInstagramUrl, socialWhatsappNumber, dealsEnabled } = useSettings();
  const tenant = useTenant();
  const displayName = tenant?.name || storeName;
  const logoSrc = tenant?.logo || "/logo.png";
  const whatsappNumber = socialWhatsappNumber || (tenant?.supportPhone || storePhone)?.replace(/\D/g, "") || "";
  return (
    <footer className="border-t bg-card/50 mt-16" data-testid="footer">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="md:col-span-1">
            <img src={logoSrc} alt={displayName} className="h-12 w-auto object-contain mb-3" />
            <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">
              {copyFooterTagline}
            </p>
            <div className="flex gap-3 mt-4">
              {socialInstagramUrl && (
                <a href={socialInstagramUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Instagram">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <Link href="/shop" className="text-sm text-muted-foreground">All Products</Link>
              {dealsEnabled && <Link href="/deals" className="text-sm text-muted-foreground">Deals & Offers</Link>}
            </div>
          </div>
        ...
        <div className="border-t mt-8 pt-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-2 text-xs text-muted-foreground">
            <p className="order-2 md:order-1">
              &copy; {new Date().getFullYear()} {displayName}. All rights reserved. Crafted with love.
            </p>
