...
import { Minus, Plus, Trash2, ShoppingBag, ArrowLeft, Shield, RotateCcw, Zap, ChevronRight, Tag, Info, X } from "lucide-react";
...
// ── Promo code section (reference: input + Apply, error alert, code tag) ───────
function PromoSection() {
  const { toast } = useToast();
  const [promoInput, setPromoInput] = useState("");
  const [applying, setApplying] = useState(false);
  const [promoError, setPromoError] = useState<string | null>(null);
  const [invalidCodeTag, setInvalidCodeTag] = useState<string | null>(null);
  const {
    appliedPromo,
    applyPromo,
    removePromo,
  ...
  const handleApply = async () => {
    ...
    setPromoError(null);
    setInvalidCodeTag(null);
    setApplying(true);
    const result = await applyPromo(promoInput);
    setApplying(false);
    if (result.success) {
      toast({ title: `"${promoInput.trim().toUpperCase()}" applied` });
      setPromoInput("");
    } else {
      const code = promoInput.trim().toUpperCase();
      setInvalidCodeTag(code);
      setPromoError(result.reason || `${code} discount code isn't valid for the items in your cart`);
    }
  };

  const dismissInvalidCode = () => {
    setPromoError(null);
    setInvalidCodeTag(null);
    setPromoInput("");
  };
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card p-4 shadow-sm">
      ...
      {/* Input + Apply (hidden when a code is applied) */}
      {!appliedPromo && (
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1 h-10 border-primary/30 focus-visible:ring-primary/50"
          />
          <Button
            size="sm"
      ...
      {/* Invalid code: dismissible alert + removable tag */}
      {promoError && (
        <>
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200/80 dark:border-amber-800/50 bg-amber-50/80 dark:bg-amber-950/40 px-3 py-2.5">
            <p className="flex-1 text-sm font-semibold text-amber-900 dark:text-amber-100">
              {promoError}
            </p>
            <button
              type="button"
              onClick={dismissInvalidCode}
              className="shrink-0 rounded p-1 text-amber-700 dark:text-amber-300 hover:bg-amber-200/50 dark:hover:bg-amber-800/50 transition-colors"
              aria-label="Dismiss"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          {invalidCodeTag && (
            <div className="mt-2 inline-flex items-center gap-2 rounded-lg border border-border bg-muted/50 px-3 py-2">
              <Tag className="w-4 h-4 text-muted-foreground shrink-0" />
              <span className="font-mono text-sm font-semibold">{invalidCodeTag}</span>
              <button
                type="button"
                onClick={dismissInvalidCode}
      ...
      {/* Applied code: compact tag with remove */}
      {appliedPromo && (
        <div className="mt-3 inline-flex items-center gap-2 rounded-lg border border-border bg-muted/50 px-3 py-2">
          <Tag className="w-4 h-4 text-primary shrink-0" />
          <span className="font-mono text-sm font-semibold">{appliedPromo.code}</span>
          {bundleItemsNeeded > 0 && (
          ...
          <Tooltip>
            ...
            <TooltipContent side="top" className="max-w-[260px] text-left">
              <p className="text-sm font-medium">{appliedPromo.title}</p>
              <p className="text-xs text-muted-foreground mt-1">{appliedPromo.description}</p>
              <p className="text-xs mt-1">{getPromoEffectText(appliedPromo)}</p>
            </TooltipContent>
          </Tooltip>
          <button
            type="button"
      ...
      {/* Available offers (when no applied code) */}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (result.success) {
                        ...
                        setInvalidCodeTag(promo.code);
                        setPromoError(result.reason || `${promo.code} isn't valid for the items in your cart`);
                      }
                    }}
                    disabled={applying}
                    className="w-full inline-flex flex-col items-start rounded-lg border border-dashed border-primary/30 bg-primary/5 px-3 py-2 text-left transition-colors hover:border-primary/50 hover:bg-primary/10 focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50 disabled:pointer-events-none"
                  >
                    <span className="font-mono text-xs font-semibold text-primary">{promo.code}</span>
                    <span className="text-[10px] text-muted-foreground mt-0.5">{getPromoEffectText(promo)}</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px]">
                  <p className="text-xs font-medium">{promo.title}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{promo.description}</p>
                  <span className="text-[11px] text-primary mt-1 block">Click to apply</span>
                </TooltipContent>
              </Tooltip>
            ))}
          </div>
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center" data-testid="page-cart-empty">
        ...
        <h1 className="font-serif text-2xl font-bold mb-2">{copyCartEmptyTitle}</h1>
